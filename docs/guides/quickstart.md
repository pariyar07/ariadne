...
2. Previews the exact synchronizer write set, obtains confirmation, then creates the canonical `00 Index.md` with a stable `scope_id`, generated-only checkpoint cores, and direct parent/child routes.
...
4. Regenerates `Bases/Scope Registry.base`, the generated block in `Agent/Scope Map.md`, and `Agent/Scope Map.canvas`. These are derived optional views; Obsidian is not required. Other Base formulas receive report-only child-before-parent ordering findings and are never authorized or rewritten implicitly.
