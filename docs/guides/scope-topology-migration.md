...
Generated titles are serialized through a minimal YAML scalar quoter, not raw interpolation. A legacy title needing both a `'` and a `"` at once — a combination this codebase's own simplified frontmatter parsers cannot safely round-trip — causes adoption to refuse with a clear error rather than emit unsafe YAML; rename the source title or use `ariadne:scope create` with an explicit title instead.
