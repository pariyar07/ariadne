...
obsidian-ingest-compile / obsidian-research-ingest
...
- obsidian-vault-validator for structural validation and deterministic health checks.
- obsidian-vault-maintainer for stewardship, stale queues, routing drift, and maintenance triage.
- obsidian-markdown for Obsidian frontmatter, wikilinks, callouts, and note formatting.
- obsidian-navigation-architect when navigation files, hubs, routing, indexes, or scope entrypoints have drifted.
- obsidian-ingest-compile or obsidian-research-ingest when inbox, raw, or source material needs compilation.
- obsidian-research-synthesis when research notes, open questions, or source captures need consolidation.
- obsidian-research-pipeline when a scope lacks a repeatable source-to-synthesis path.
- obsidian-bases when Base views, Base indexes, filters, or base-file links need repair.
- obsidian-vault-discovery when registry, vault entrypoints, or discovery metadata look stale.
...
2. Run the validator script from obsidian-vault-validator and capture the exact result.
