...
- Generated descriptor frontmatter (`title`, `scope_path`, `former_scope_paths`, and move's redirect `former_scope_path`/`target_scope_path`) is now serialized through a minimal, dependency-free YAML scalar quoter rather than raw string interpolation. Every generated string scalar is now quoted unconditionally rather than only when a resolver regex judged it necessary -- an earlier revision tried enumerating YAML 1.1 implicit-type forms (booleans, null, numeric-looking, hex-looking, ISO-date-looking) and quoting only those, but any omitted form (e.g. `01`, `1.`, `.inf`, `.NaN`, `0b101`, `12:34`) silently produced a value a real, standards-compliant YAML parser would coerce away from a string while this codebase's own naive parser still round-tripped it correctly, so the bug stayed invisible to this repository's own tests. A value needing both `'` and `"` simultaneously, a backslash, or a control character -- combinations this codebase's own simplified parsers cannot safely round-trip -- causes adoption to refuse with a clear error instead of emitting unsafe or corrupt YAML. A folder path that violates the schema's own existing Windows-compatibility character restrictions (`normalizeScopePath`, e.g. a literal `:` in a segment), or whose normalized form differs from its actual path (e.g. a POSIX-legal directory literally named with a backslash, which `normalizeScopePath` silently rewrites to a forward slash rather than rejecting), is also refused at discovery, since it could never be re-read as -- or reliably re-authorized against -- the same scope_path afterward.
...
- `buildTopology`'s ancestor-resolution pass sorted candidate descriptors by directory depth using `directory.split("/").length`, which counts root's own directory (`"."`) as depth 1 -- tied with any real top-level folder -- so root could be processed after one of its own children whenever alphabetical file-scan order placed that child's file first (any folder name sorting before `"0"`, e.g. one starting with `#`). The child's ancestor lookup then found nothing yet registered and was silently dropped from the topology entirely. Root now always sorts at depth 0.
...
- `yamlScalar`'s control-character refusal, and the schema's shared `CONTROL_CHARACTERS` check, now match the full Unicode `Cc` (Control) category instead of only the C0 range (U+0000-U+001F, U+007F). C1 control characters (U+0080-U+009F) were previously allowed through unquoted or quoted; `U+0085` (NEL) in particular has line-break semantics in a real, standards-compliant YAML parser and folds to a space, so a title containing it would silently change value across the naive/real-parser boundary even though this codebase's own line-based parser (which only recognizes `\r\n`/`\n`) round-tripped it unchanged.
- `inventoryVault` now also records each file's and directory's pre-normalization `rawRelativePath` alongside the always-NFC-normalized `relativePath`, and legacy discovery refuses a candidate directory whose `rawRelativePath` differs from its NFC form. `relativePath`'s NFC normalization made equal-looking paths compare equal regardless of encoding, but on a filesystem that does not itself normalize on write (e.g. ext4, unlike APFS) a directory's physical bytes can be NFD-decomposed while its `relativePath` reads back composed; discovery previously had no way to detect that divergence and could authorize a scope_path the filesystem doesn't actually have under that spelling.
