...
- Generated descriptor frontmatter (`title`, `scope_path`, `former_scope_paths`, and move's redirect `former_scope_path`/`target_scope_path`) is now serialized through a minimal, dependency-free YAML scalar quoter rather than raw string interpolation. Values containing colons, leading `#`/`{`/`-`, YAML 1.1 implicit types (booleans, null, numeric-looking, hex-looking, ISO-date-looking, in any case), or quote characters are now quoted correctly; a value needing both `'` and `"` simultaneously, a backslash, or a control character -- combinations this codebase's own simplified parsers cannot safely round-trip -- causes adoption to refuse with a clear error instead of emitting unsafe or corrupt YAML. A folder path that violates the schema's own existing Windows-compatibility character restrictions (`normalizeScopePath`, e.g. a literal `:` in a segment) is also refused at discovery, since it could never be re-read as a valid descriptor afterward.
