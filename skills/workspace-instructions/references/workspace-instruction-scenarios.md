...
- Retired research skill names inside well-formed Ariadne-managed marker blocks, with exact successor mappings and marker-bounded repair guidance. Ordinary user text outside those blocks is intentionally not flagged or rewritten.
...
| Retired research skill names | Replace retired research identifiers with the checker's exact successor mappings only inside well-formed Ariadne-managed marker blocks. | Ask before changing the same text outside managed blocks or when markers are malformed or duplicated. | Do not rewrite user-authored prose, saved prompts, or external scheduler configuration from a workspace scan. |
...
| Markdown knowledge-vault root, including Obsidian-compatible vaults | Defer to `ariadne:vault` conventions unless only adding a compact compatible vault link. | Ask before replacing vault navigation or agent files with generic workspace templates. | Do not treat a knowledge-vault root as a generic repo. |
