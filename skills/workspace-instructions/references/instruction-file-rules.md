...
| B8 | Stable scope identity: a scope link has exactly one lower-kebab ID, one safe normalized vault-relative NFC path, and one equal linked target. With a user-confirmed vault, raw supported descriptors plus the shared topology model must prove a unique collision-free canonical mapping; ambiguity is never reported as current/stale. Without a vault, valid identities stay explicitly unverified. Repair is bounded to the marker. | [M] | `workspaceScopeIdentityByFile`, `invalidScopeIdentityFiles`, `unverifiedScopeIdentityFiles`, `ambiguousScopeIdFiles`, `unknownScopeIdFiles`, `staleScopePathFiles`, `scopeIdentityVaultConfirmed`, `scopeLinkRepairGuidance` |
