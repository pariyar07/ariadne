...
- If the target scope has no research pipeline, use `obsidian-research-pipeline` first.
- If the target scope has intake infrastructure and this is one source, use `obsidian-ingest-compile`.
- If the source updates an ongoing multi-source argument, use `obsidian-research-synthesis` after ingesting.
...
- If the source is not research and is just an operational note, use `obsidian-ingest-compile` directly.
...
5. If the pipeline is missing, create it with `obsidian-research-pipeline`.
...
- run `obsidian-vault-validator` after structural changes
