...
If more than one plausible candidate remains, show the top matches with short reasons and ask which one to resume before mutating anything. Do not infer the user's choice from priority, card wording, "real" versus "temporary" labels, decoy/test labels, apparent importance, recency, current board order, or your belief about which workstream is intended. A decoy or temporary card is still an active candidate until its `workstream_status` is changed to `closed` or the user explicitly excludes it in the current turn. Resume only when the current user prompt names the exact workstream id/name or the user chooses one after you ask.
